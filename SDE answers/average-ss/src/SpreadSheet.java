...
public class SpreadSheet extends JFrame {
    ...
    public String evaluateToken(String tok, int depth) {
        ...
        /* if token is ref : length() >=2 & charAt(0) >= 'A' & < 'K' */
        /* checking col */
        System.out.println("token " + tok);
        if ( tok.length() >= 2 && tok.charAt(0) >= 'A' && 
            tok.charAt(0) < (char) ('A' + maxCols)) {
            
            /* if ref is A1 for example */
            /* col is 0, first column */
            ...
            if (row >= 0 && row < maxRows) {
                ...
                /* if the current cellTF doensn't has correct current value */
    ...
    protected String average(ArrayList<String> aveList, int depth)throws NumberFormatException {
        double averages = 0;
        
        for(String ave : aveList){
            String average = evaluateToken(ave, depth);
            if(average == null) return null;
            averages += Double.parseDouble(average.trim());
        }
        
        return Double.toString(averages/aveList.size());
    }
    ...
    public String parseFormula(StringTokenizer tokens, int depth) 
        ...
        if (tokens.hasMoreTokens()) {
            ...
            String avgStart = "";
            String avgEnd = "";
            if( tok.equals("AVG") && tokens.hasMoreTokens()){
                tok = tokens.nextToken();
                if(tok.equals("(") && tokens.hasMoreTokens()){
                    tok = tokens.nextToken();
                    avgStart = tok;
                }
            }
            //System.out.println("before evaluate token " + tok);
            tok = evaluateToken(tok, depth);
            //System.out.println("after evaluate token " + tok);
            ...
            while (tokens.hasMoreTokens()) {
                ...
                if (tok2.equals("+")) {
                	...
                	tok = subtract(tok, tok3);
                        ...
                        ArrayList<String> aveList = new ArrayList();
                        char startCol = avgStart.charAt(0); 
                        char endCol = avgEnd.charAt(0); 
                        int startRow = Integer.parseInt(avgStart.substring(1));
                        int endRow = Integer.parseInt(avgEnd.substring(1)); 
                        if((startCol == endCol) || startRow == endRow){
                            //System.out.println("same row, same column test");
                            if(startCol > endCol){
                                char help = endCol;
                                endCol = startCol;
                                startCol = help;
                            ...
                            if(startCol == endCol){
                                ...
                                for(int i = startRow; i <= endRow; i++){
                                    String avgEntry = startCol + String.valueOf(i);
                                    //System.out.println(avgEntry);
                                    aveList.add(avgEntry);
                                }
                            } else {
                                //System.out.println("same row");
                                for(int i = 0 + startCol; i <= 0 + endCol; i++){
                                    String avgEntry = String.valueOf((char)i) + String.valueOf(startRow);
                                    //System.out.println(avgEntry);
                                    aveList.add(avgEntry);
                                }
                            }
                            tok = average(aveList, depth);
    ...
    public void evaluate(int r, int c, int depth) {
        ...
        //System.out.println("evaluate's formula " + formula);
            ...
            try {
                if (depth <= maxRows * maxCols) {
                    ...
                    if (tokens.hasMoreTokens() && 
                        ...
                        /* if calculated value is not null */
                        if (val != null) {
