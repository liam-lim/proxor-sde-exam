...
public class SpreadSheet extends JFrame {
    ...
    public String evaluateToken(String tok, int depth) {
        if (tok.length() >= 2 && tok.charAt(0) >= 'A' && 
            ...
            if (row >= 0 && row < maxRows) {
                ...
                if (cells[row][col].trancated) return cells[row][col].formula;
                return cellsTF[row][col].getText();
            }
        }
        return null;
    ...
    public void format(String formula, int r, int c){
        try{
            Formatter fmt = new Formatter();
            double doubleFormula = Double.parseDouble(formula.trim());
            if(doubleFormula < 1.0e+7 && doubleFormula >= 1.0)// if less than 10^7... eg.1234567.8 (10^6 < 10^7)
                fmt.format("%.7g", doubleFormula);//for decimal rounding
                                                    //for g .7 is also decimal place
            else                       
                fmt.format("%.2e", doubleFormula);// for scientific notation
                                                    // for e .2 is decimal place
            cellsTF[r][c].setText(fmt.toString());
            System.out.println("cell[" + r + "]" + "[" + c + "] " + cells[r][c].formula);
    ...
    public void cellChanged(JTextField cell) {
        ...
        if (f.length() <= 0 || f.charAt(0) != '=') {
            // update the formula if its length not greater than 8
            if(!cells[r][c].trancated){
                cells[r][c].formula = cell.getText();
                //cells[r][c].realFormula = cell.getText();
