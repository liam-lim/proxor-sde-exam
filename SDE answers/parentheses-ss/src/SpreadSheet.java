    ...
    public String parseFormula(StringTokenizer tokens, int depth) 
        ...
        if (tokens.hasMoreTokens()) {
            ...
            while (tokens.hasMoreTokens()) {
                ...
                if(tok3.startsWith("(")){
                    String subFormula = tok3.substring(1);
                    while(tokens.hasMoreTokens()){
                        String subTok = tokens.nextToken();
                        if(!subTok.endsWith(")")){
                            subFormula += subTok;
                        }else{
                            subFormula += subTok.substring(0, subTok.length()-1);
                            break;
                        }                      
                    } 
                    StringTokenizer subTokens = 
                            new StringTokenizer(subFormula, "=+*/-", true);
                    String val = parseFormula(subTokens, depth);
                    tok3 = val;
                ...
                System.out.println("tok3 : " + tok3 + " when tok : " + tok);
