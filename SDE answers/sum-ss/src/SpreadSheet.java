...
public class SpreadSheet extends JFrame {
    ...
    protected String sum(ArrayList<String> sumList, int depth)throws NumberFormatException {
        double sums = 0;
        
        for(String sum : sumList){
            String tok = evaluateToken(sum, depth);
            if(tok == null) return null;
            sums += Double.parseDouble(tok.trim());
        }
        
        return Double.toString(sums);
    }
    ...
    public String parseFormula(StringTokenizer tokens, int depth) 
        ...
        if (tokens.hasMoreTokens()) {
            ...
            if( tok.equals("SUM") && tokens.hasMoreTokens()){
                tok = tokens.nextToken();
                if(tok.equals("(") && tokens.hasMoreTokens()){
                    tok = tokens.nextToken();//tok is A1 as in =SUM(A1:C1)
                    sumStart = tok;
                }
            }
            ...
            while (tokens.hasMoreTokens()) {
                ...
                if (tok2.equals("+")) {
                	...
                	tok = subtract(tok, tok3);
                        ...
                        char startCol = sumStart.charAt(0); 
                        char endCol = sumEnd.charAt(0); 
                        int startRow = Integer.parseInt(sumStart.substring(1));
                        int endRow = Integer.parseInt(sumEnd.substring(1)); 
                        ...
                        if((startCol == endCol) || startRow == endRow){
                            ...
                            if(startCol == endCol){
                                ...
                                for(int i = startRow; i <= endRow; i++){
                                    String sumEntry = startCol + String.valueOf(i);
                                    //System.out.println(avgEntry);
                                    sumList.add(sumEntry);
                                }
                            } else {
                                //System.out.println("same row");
                                for(int i = 0 + startCol; i <= 0 + endCol; i++){
                                    String sumEntry = String.valueOf((char)i) + String.valueOf(startRow);
                                    //System.out.println(avgEntry);
                                    sumList.add(sumEntry);
                                }
                            }
                            tok = sum(sumList, depth);
